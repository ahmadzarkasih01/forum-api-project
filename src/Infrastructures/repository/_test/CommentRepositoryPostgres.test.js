...
describe("CommentRepositoryPostgres", () => {
  ...
  describe("verifyCommentIsExist function", () => {
    it("should throw NotFound Error when comment not found", async () => {
      ...
      // Action & Assert
      await expect(commentRepositoryPostgres.verifyCommentIsExist("comment-123")).rejects.toThrowError(NotFoundError);
    });
    ...
    it("should not throw Not Found Error when comment is owned", async () => {
      ...
      // Action & Assert
      await expect(commentRepositoryPostgres.verifyCommentIsExist(commentId)).resolves.not.toThrowError(NotFoundError);
    });
  });
  ...
  describe("verifyCommentOwner function", () => {
    it("should throw Authorization Error when comment was not owned", async () => {
      ...
      // Action & Assert
      await expect(commentRepositoryPostgres.verifyCommentOwner(commentId, userId)).rejects.toThrowError(AuthorizationError);
    });
  ...
  describe("getThreadComments function", () => {
    it("should get thread comments correctly", async () => {
      ...
      expect(new Date(comments[0].date).toISOString()).toEqual("2021-08-08T07:22:33.555Z");
      ...
      expect(new Date(comments[1].date).toISOString()).toEqual("2021-08-08T07:26:21.338Z");
